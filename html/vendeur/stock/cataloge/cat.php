...
class PDF extends FPDF
{
    ...
    function BasicTable($data)
    {
        ...
        foreach ($data as $row) {
                $x = $this->GetX();
                $y = $this->GetY();
                $this->Image(HOME_SITE . htmlentities($row['url_image']), null,null,0, 20);
                $this->SetX(20);
                $this->SetY($y);
                $this->Cell(40, 20, htmlentities($row['nom_public']), 1, 0, 'c');
                $this->Cell(40, 20, round($row['prix'],2). htmlentities(" €"), 1, 0, 'c');
            $this->Ln();
        }
    }
