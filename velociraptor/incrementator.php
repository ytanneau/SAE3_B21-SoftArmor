<?php 
    ...
    foreach ($ret as $key => $tab) {
        ...
        //on augmente l'etape de 1
            ...
            $etape=$tab['etape']+1;
            $sql = "UPDATE _colis SET etape= :etape,date_update = NOW() where bordereau = :bordereau";
            $stmt = $pdo->prepare($sql);
            ...
            $etape=$tab['etape']+1; 
            $est_livre= rand(0,2);
            $sql = "UPDATE _colis SET etape= :etape,date_update = NOW() ,mode=:mode where bordereau = :bordereau";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([':etape' => $etape,':mode' => $est_livre,':bordereau' => $tab['bordereau']]);
            //s'il il est pas livré choisir raison de refus
            if($est_livre == 2){
                $raison_refus = rand(0,3);
                $sql = "UPDATE _colis SET raison_refus= :raison_refus,date_update = NOW() where bordereau = :bordereau";
                $stmt = $pdo->prepare($sql);
